...
for i in tqdm(range(BLOCK_COUNT), desc="Fetching blocks", unit="block"):
    ...
    try:
        ...
        print(f"  Failed to fetch block header for height {height}: {e}")
    ...
    if block_header or block_results or block_validators:
        output = {
            "base_url": *__*,
            "blockheader": *__*,
            "blockresults": *__*,
        ...
        filename = os.path.join(SAVE_DIR, f"{BASE_NAME}_BlockNum_{height}.json")
