...
この README では、`./setup/...`、`Day1/...`、`requirements.txt` のような相対パスを使うコマンドは、原則としてリポジトリルートで実行する。
標準のリポジトリルートは `~/temp/sus_BC2` である。

```bash
cd ~/temp/sus_BC2
```

対応する setup script: なし。作業場所をリポジトリルートへ移動するための基本コマンドである。

`wsl ...` コマンドは、Windows 側のコマンドプロンプトで実行する。
`cd ~/temp`、`cd ~/temp/simblock`、`cd ~/temp/cw-plus` が入っているコマンドブロックは、そのブロック内で必要な実行ディレクトリへ移動している。
...
対応する setup script: なし。Windows のコマンドプロンプトで実行する WSL 設定コマンドである。
...
対応する setup script: なし。Windows のコマンドプロンプトで Kali Linux を起動するコマンドである。
...
Windows のスタートメニューで `cmd` を検索し、右クリックして「管理者として実行」を選ぶ。
本 README ではコマンドプロンプトを使う。

### 3. WSL と Kali Linux をインストールする

管理者として開いたコマンドプロンプトで次のコマンドを実行する。

```cmd
wsl --install --distribution kali-linux
```

対応する setup script: なし。Windows のコマンドプロンプトで実行する WSL / Kali Linux インストールコマンドである。
...
Windows のコマンドプロンプトから接続できるか確認する。
...
対応する setup script: なし。Windows のコマンドプロンプトから SSH 接続を確認するコマンドである。
...
Kali Linux の WSL 環境を削除して作り直す場合は、コマンドプロンプトで次のコマンドを実行する。
